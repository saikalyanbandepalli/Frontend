...
const ResumeForm = ({ userId }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const request = mode === 'create' ? axios.post(url, formDataToSend) : axios.put(url, formDataToSend);
    ...
    <div>
      ...
      {mode === 'view' ? (
        ...
        <form onSubmit={handleSubmit} className="resume-form">
          ...
          {formData.experience.map((exp, index) => (
            <div key={index}>
              ...
              <input
                ...
                required
              />
              <p>Experience: {calculateExperience(exp.startDate, exp.endDate)}</p>
            </div>
          ))}
          {mode !== 'view' && (
          ...
          <h3>Upload Photo</h3>
          <input type="file" accept="image/*" onChange={handleFileChange} disabled={mode === 'view'} />
  ...
  return (
    <div className="resume-view">
      ...
      {data.education.map((edu, index) => (
        <div key={index}>
          <p>{edu.degree} at {edu.institution} ({edu.startYear} - {edu.endYear})</p>
        </div>
      ))}
      ...
      {data.experience.map((exp, index) => (
        <div key={index}>
          <p>{exp.jobPosition} at {exp.officeName} ({calculateExperience(exp.startDate, exp.endDate)})</p>
        </div>
      ))}
